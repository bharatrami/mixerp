...
namespace MixERP.Net.DatabaseLayer.Transactions
{
    public static class SalesDelivery
    {
        ...
        public static long Add(DateTime valueDate, int officeId, int userId, long logOnId, int storeId, int costCenterId, string referenceNumber, string statementReference, MixERP.Net.Common.Transactions.Models.StockMasterModel stockMaster, Collection<MixERP.Net.Common.Transactions.Models.StockMasterDetailModel> details)
        {
            if(stockMaster == null)
            {
                return 0;
            ...
            using(NpgsqlConnection connection = new NpgsqlConnection(DBFactory.DBConnection.ConnectionString()))
            {
                ...
                using(NpgsqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        sql = "INSERT INTO transactions.stock_master(stock_master_id, transaction_master_id, party_id, price_type_id, is_credit, shipper_id, shipping_charge, store_id) SELECT nextval(pg_get_serial_sequence('transactions.stock_master', 'stock_master_id')), @TransactionMasterId, core.get_party_id_by_party_code(@PartyCode), @PriceTypeId, @IsCredit, @ShipperId, @ShippingCharge, @StoreId; SELECT currval(pg_get_serial_sequence('transactions.stock_master', 'stock_master_id'));";
